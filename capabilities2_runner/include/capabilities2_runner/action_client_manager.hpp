...
public:
	...
	void init_action(const std::string& action_name)
	{
		...
		{
			RCLCPP_ERROR(node_->get_logger(), "%s failed to connect to action: %s", run_config_.interface.c_str(), action_name.c_str());
			throw runner_exception("failed to connect to action server");
		}

		// send goal options
		// goal response callback
		goal_options_.goal_response_callback = [this, &goal_handle_](const typename rclcpp_action::ClientGoalHandle<ActionT>::SharedPtr& goal_handle) 
		{
		...
		// result callback
		goal_options_.result_callback = [this](const typename rclcpp_action::ClientGoalHandle<ActionT>::WrappedResult& wrapped_result) 
		{
			if (wrapped_result.code == rclcpp_action::ResultCode::SUCCEEDED)
			{
				// publish event
				if (on_started_) on_started_(run_config_.interface);
			}
			else
			{
				// send terminated event
				if (on_terminated_) on_terminated_(run_config_.interface);
			}
		};
